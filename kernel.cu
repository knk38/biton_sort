...
__global__ void MatrixMulCUDA(float* C, float* A, float* B, int matrixWidth) 
{
    ...
    float value = 0.0f;
    for (int t = 0; t < (matrixWidth + TILE_WIDTH - 1) / TILE_WIDTH; ++t) {
        if (row < matrixWidth && (t * TILE_WIDTH + threadIdx.x) < matrixWidth) {
            tileA[threadIdx.y][threadIdx.x] = A[row * matrixWidth + t * TILE_WIDTH + threadIdx.x];
        } else {
            tileA[threadIdx.y][threadIdx.x] = 0.0f;
        }
        if (col < matrixWidth && (t * TILE_WIDTH + threadIdx.y) < matrixWidth) {
            tileB[threadIdx.y][threadIdx.x] = B[(t * TILE_WIDTH + threadIdx.y) * matrixWidth + col];
        } else {
